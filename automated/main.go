...
func main() {
	...
	for i, ap := range analyzeParams {
		...
		appendResultCSV([]string{
			fmt.Sprintf("%s", startBenchmark.Format("2006-01-02 15:04:05")),
			fmt.Sprintf("%s", endBenchmark.Format("2006-01-02 15:04:05")),
		...
		go func(start <-chan struct{}) {
			...
			for _, stock := range stocks {
				execDurs, analyzeRes := func(stockCode string, iteration int) (execDuration []float64, result []float64) {
					query := fmt.Sprintf(`explain analyse select * from high_load_prototyping where porto_stock_code = '%s' AND has_credit = true`, stockCode)

					res := runExplainAnalyze(db, query)
					result = append(result, res)

					querySelect := fmt.Sprintf(`select * from high_load_prototyping where porto_stock_code = '%s' AND has_credit = true`, stockCode)
...
func runExplainAnalyze(db *sqlx.DB, query string) float64 {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&explainRow); err != nil {
			panic(err)
		}

		// check if the row contains the string "Execution Time"
		if strings.Contains(explainRow, "Execution Time") {
			match := re.FindString(explainRow)
